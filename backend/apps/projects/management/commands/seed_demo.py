...
PROJECTS = [
    ...
    },
    {
        ...
        "overview": "My_Torch is a small neural-network engine built without any ML framework — PyTorch, TensorFlow and similar libraries are explicitly forbidden by the subject — split into a reusable NN module (create, save/load, train a network) and a my_torch_analyzer executable that classifies a chessboard given in FEN notation as Checkmate, Check or Nothing.",
        "problem": "Detect the state of a chess game — Checkmate, Check or Nothing, and optionally which side has the advantage — from a chessboard given in Forsyth-Edwards Notation, using a model trained purely with supervised learning.",
        "contribution": "Entire project: the neural-network module (layer creation, forward pass, save/load, training) and the my_torch_analyzer CLI (train and predict modes).",
        "stack": "C++, custom matrix operations, hand-implemented gradient descent.",
        "architecture": "A configurable neural-network module exposing creation, save/load and training; and a my_torch_analyzer executable with two modes — --train (learns from a labelled FEN dataset, optionally saved with --save) and --predict (loads a saved network and classifies every board in an input file, in the same order).",
        "results": "[TODO: classification accuracy reached on Checkmate / Check / Nothing, and on the White/Black-advantage bonus if you implemented it.]",
        ...
        "challenges": [
            ("Avoiding overfitting", "The subject makes this a hard requirement. [Describe the validation/regularisation strategy you used to keep the network from memorising the training set.]"),
            ("Numerical stability", "[Describe issues with exploding/vanishing values and how you handled them.]"),
        ],
    },
    {
        "slug": "gomoku",
        ...
        "overview": "A Gomoku (five-in-a-row) bot — binary name pbrain-gomoku-ai — playing on a 20x20 board under the freestyle ruleset, driven by the liskvork game manager and speaking the Epitech Mirror communication protocol.",
        "problem": "The search space on a 20x20 board is huge, and only standard libraries are allowed — no TensorFlow, no scikit-learn. Each move must come back within 5 seconds and 70MB of memory, so the AI needs both a strong position-evaluation heuristic and a search that respects that budget.",
        ...
        "results": "Evaluated by Epitech's two-step process: solving pre-filled 'winning move' boards, then playing matches against local bots of low/medium level. [TODO: win rate reached against those reference bots.]",
        ...
        "challenges": [("Staying within the 5-second, 70MB-per-move budget", "[Describe the iterative-deepening / early-cutoff strategy that kept the search inside this limit.]")],
    },
]
...
SCHOOL_PROJECTS = [
    ...
    },
    {
        ...
        "code": "G-DOP-400",
        "module": "DevOps",
        "pitch": "Configured a whole Jenkins instance as code — no manual clicking — with role-based permissions and jobs that clone and test a repository on every push.",
        "details": "Everything lives in two files: `my_marvin.yml` (Jenkins Configuration as Code) sets up users, a role-based authorization strategy with four distinct roles, and a Tools folder; `job_dsl.groovy` defines a SEED job that, given a GitHub repo, generates a job polling it every minute and running fclean/make/tests_run/clean as separate shell steps. Only the required plug-ins are installed, and no password is ever hardcoded — every one comes from an environment variable.",
        "order": 20,
    ...
    },
    {
        ...
        "code": "G-DOP-400",
        "module": "DevOps",
        "pitch": "Deployed the Popeye poll app across 5 bare VMs with Ansible — no containers this time — using six idempotent roles.",
        "details": "A `playbook.yml` and six roles (base, redis, postgresql, poll, worker, result) provision Debian machines by inventory group, installing and configuring each service with dedicated Ansible modules rather than raw shell commands. Services run under systemd and are configured through environment variables per the 12-factor rules; running the playbook twice produces zero changed tasks — full idempotence, with no clear-text password allowed anywhere in the repository.",
        "order": 21,
    ...
    },
    {
        ...
        "code": "G-DOP-500",
        "module": "DevOps",
        "pitch": "Orchestrated the same poll app on Kubernetes: a load balancer, two databases and three services, with Traefik routing and cAdvisor monitoring.",
        "details": "Kubernetes manifests define Redis and PostgreSQL (the latter with a persistent volume), the poll/worker/result services, a replicated Traefik load balancer routing poll.dop.io and result.dop.io to the right service, and a cAdvisor DaemonSet on every node for container metrics. Only the database credentials sit in a Secret; every other setting lives in a ConfigMap, and replicated pods are spread across different nodes for availability.",
        "order": 22,
    ...
    },
    {
        ...
        "code": "G-DOP-500",
        "module": "DevOps",
        "pitch": "The DevOps capstone: a Jenkins-driven platform that detects a pushed repo's language, containerises it into a Whanos base image, and deploys it straight to a Kubernetes cluster.",
        "details": "Combines every tool from the module — Docker, Jenkins, Ansible and Kubernetes. A `link-project` job registers a repository; on every push, a generated Jenkins job detects the language (C, Java, JavaScript, Python or Befunge), builds it into the matching Whanos base image, pushes it to a registry, and — if the repo ships a `whanos.yml` — deploys it into the cluster with the requested replica count, resources and ports. The Jenkins infrastructure itself is stood up with Ansible.",
        "order": 23,
